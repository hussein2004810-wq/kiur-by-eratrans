...
for(const file of ['drizzle/0000_medexam.sql','drizzle/0001_academic_hierarchy.sql','drizzle/0002_backfill_existing_tests.sql','drizzle/0003_scale_indexes.sql','drizzle/0004_attempt_shuffle.sql'])sqlite.exec(await readFile(file,'utf8'));
...
const students=await call('/api/admin/students?limit=50',{id:'admin-1',role:'admin'});if(students.status!==200||students.data.total!==1||Number(students.data.data[0].averagePercentage)!==33.33)throw new Error('Student report verification failed');
