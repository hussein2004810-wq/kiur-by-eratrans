...
const hub=await call('/api/me/learning-hub');if(hub.status!==200||hub.data.activeAttempt?.attemptId!=='learning-active'||hub.data.favorites?.[0]?.id!=='demo-preop'||Number(hub.data.weakTopics?.[0]?.wrongAnswers)!==1)throw new Error(`Learning hub response failed: ${JSON.stringify(hub)}`);
...
console.log(JSON.stringify({ok:true,resume:hub.data.activeAttempt.title,favorites:hub.data.favorites.length,weakTopics:hub.data.weakTopics.length,ownerDenied:denied.status}));
