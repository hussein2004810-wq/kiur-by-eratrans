...
export function createFirebaseAuthMock(){
  ...
  const api={
    async signUp({email,password}){const key=normalize(email);if(users.has(key))throw new FirebaseAuthError('EMAIL_EXISTS');const user={uid:`fb-${crypto.randomUUID()}`,email:key,password,emailVerified:false,disabled:false,providers:new Set(['password'])};users.set(key,user);return issue(user)},
    ...
    async lookup({idToken}){const user=tokens.get(idToken);if(!user)throw new FirebaseAuthError('INVALID_ID_TOKEN');return {users:[{localId:user.uid,email:user.email,displayName:user.name||'',emailVerified:user.emailVerified,disabled:user.disabled,providerUserInfo:[...(user.providers||[])].map(providerId=>({providerId}))}]}},
  ...
  return {
    ...
    markGoogle(email,name='Google student'){const user=users.get(normalize(email));if(!user)throw new Error('Firebase mock user not found');user.providers.add('google.com');user.name=name},
