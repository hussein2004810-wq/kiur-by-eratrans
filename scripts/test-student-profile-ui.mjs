...
assert.match(profile,/accept="image\/png,image\/jpeg,image\/webp"/);assert.match(profile,/2\*1024\*1024/);assert.match(profile,/\/api\/me\/profile-dashboard/);assert.match(profile,/\/api\/me\/sessions/);assert.match(profile,/CertificateButton/);assert.match(app,/view==='profile'/);assert.match(app,/فتح بروفايل الطالب/);assert.match(css,/@media\(max-width:640px\)/);assert.match(css,/@media\(prefers-reduced-motion:reduce\)/);assert.doesNotMatch(profile,/dangerouslySetInnerHTML/);
