...
async function clinicalMediaIssue(env,user,questions,path){const ids=[...new Set(questions.filter(question=>question.questionType==='clinical_case').map(question=>question.imageId))];if(!ids.length)return null;if(!(await hasPermission(env,user,'use_media',pathContext(path))))return 'لا تملك صلاحية استخدام مكتبة الصور';for(const id of ids){const asset=await env.DB.prepare(`SELECT id FROM media_assets WHERE id=? AND deleted_at IS NULL`).bind(id).first();if(!asset)return 'إحدى صور الحالات السريرية غير موجودة أو محذوفة'}return null}
...
async function catalog(env){
  const [universities,colleges,departments,phases,sections,subjects,lectures]=await Promise.all([
    ...
    env.DB.prepare(`SELECT id,college_id AS collegeId,name,sort_order AS sortOrder FROM departments ORDER BY sort_order,name`).all(),
...
export async function handleHierarchyApi(request,env,url,user){
  ...
  if(adminTest&&request.method==='PUT'){
    ...
    value.questions.forEach((question,index)=>statements.push(env.DB.prepare(`INSERT INTO questions(id,test_id,text,options_json,correct_option,explanation,position,question_type,accepted_answers_json,image_id) VALUES(?,?,?,?,?,?,?,?,?,?)`).bind(crypto.randomUUID(),adminTest[1],question.text.trim(),JSON.stringify((question.options||[]).map(String)),Number(question.correctOption||0),question.explanation?.trim()||null,index+1,question.questionType||'mcq',question.questionType==='fill_blank'?JSON.stringify(question.acceptedAnswers.map(String)):null,question.imageId||null)));
