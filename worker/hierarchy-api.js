...
function validateQuestions(value){
  ...
  for(const question of value.questions){
    ...
    if(question.points!==undefined&&(typeof question.points!=='number'||question.points<=0||question.points>100))return 'درجة أحد الأسئلة غير صالحة';
  }
  return null;
}
...
function publishedNotification(env,testId,title,path){return env.DB.prepare(`INSERT OR IGNORE INTO user_notifications(id,user_id,notification_type,title,message,link) SELECT 'test-published:'||?||':'||u.id,u.id,'test_published','اختبار جديد في مرحلتك',?,? FROM users u WHERE u.account_role='student' AND u.account_status='active' AND u.university_id=? AND u.college_id=? AND u.department_id=? AND u.phase_id=? AND (? IS NULL OR u.section_id=?)`).bind(testId,title,`/test/${encodeURIComponent(testId)}`,path.universityId,path.collegeId,path.departmentId,path.phaseId,path.sectionId,path.sectionId)}
...
export async function handleHierarchyApi(request,env,url,user,restriction=null){
  ...
  if(url.pathname==='/api/admin/tests'&&request.method==='POST'){
    ...
    await env.DB.batch(statements);if((value.status||'published')==='published')await publishedNotification(env,id,value.title.trim(),path).run();return json({id},201);
  }
  ...
  if(adminTest&&request.method==='GET'){
    ...
    const questions=await env.DB.prepare(`SELECT id,text,options_json,correct_option AS correctOption,explanation,position,question_type AS questionType,accepted_answers_json AS acceptedAnswers,image_id AS imageId,points FROM questions WHERE test_id=? ORDER BY position`).bind(test.id).all();
    return json({...test,certificateEnabled:Boolean(test.certificateEnabled),questions:questions.results.map(question=>({...question,options:JSON.parse(question.options_json),acceptedAnswers:question.acceptedAnswers?JSON.parse(question.acceptedAnswers):[],points:Number(question.points||1)}))});
  }
  ...
  if(adminTest&&request.method==='PUT'){
    ...
    const parsed=await readBody(request);if(parsed.error)return fail(parsed.error.code,parsed.error.message,parsed.error.status);const value=parsed.value;const issue=validateQuestions(value);if(issue)return fail('VALIDATION',issue);
    const path=await resolvePath(env,value);if(!path)return fail('VALIDATION','المسار الأكاديمي المختار غير صالح');
    const exists=await env.DB.prepare(`SELECT t.id,t.status,t.department_id AS departmentId,t.phase_id AS phaseId,t.section_id AS sectionId,t.subject_id AS subjectId,t.lecture_id AS lectureId,c.university_id AS universityId,d.college_id AS collegeId FROM tests t LEFT JOIN departments d ON d.id=t.department_id LEFT JOIN colleges c ON c.id=d.college_id WHERE t.id=? AND t.status!='archived'`).bind(adminTest[1]).first();
    ...
    await env.DB.batch(statements);if(exists.status!=='published'&&(value.status||'published')==='published')await publishedNotification(env,adminTest[1],value.title.trim(),path).run();return json({updated:true});
  }
