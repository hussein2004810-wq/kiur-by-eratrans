...
export default function AccountManager({catalog,currentRole,notify}:{catalog:Catalog;currentRole:Role;notify:(message:string)=>void}){
  ...
  const submit=async(event:React.FormEvent)=>{event.preventDefault();setBusy(true);try{const grants=[{scopeType:form.scopeType,scopeId:form.scopeType==='platform'?'platform':form.scopeId,permissions:form.permissions}];const payload={...form,staffTitle:form.role==='teacher'?form.staffTitle:null,grants};if(editing)await request(`/api/admin/users/${editing.id}/grants`,{method:'PUT',body:JSON.stringify(payload)});else await request('/api/admin/users',{method:'POST',body:JSON.stringify(payload)});notify(editing?'تم تحديث صفة الكادر والصلاحيات':'تم إنشاء حساب الكادر وتفعيل صلاحياته');reset();await load()}catch(error){notify((error as Error).message)}finally{setBusy(false)}};
  ...
  return <section className="accountsManager">
    ...
    <div className="accountGrid">
      <form className="panel accountForm" onSubmit={event=>void submit(event)}>
        ...
        {!editing&&<><input required minLength={2} placeholder="اسم عضو الكادر أو المشرف" value={form.name} onChange={event=>setForm({...form,name:event.target.value})}/><input required type="email" placeholder="البريد الإلكتروني" value={form.email} onChange={event=>setForm({...form,email:event.target.value})}/><input required type="password" minLength={10} placeholder="كلمة مرور مؤقتة قوية" value={form.password} onChange={event=>setForm({...form,password:event.target.value})}/></>}
        ...
        <div className="accountActions">{editing&&<button type="button" onClick={reset}>إلغاء</button>}<button className="solid" disabled={busy||!form.permissions.length||!form.scopeId}><Plus/>{busy?'جارٍ الحفظ...':editing?'حفظ الصفة والصلاحيات':'إنشاء الحساب'}</button></div>
      </form>
