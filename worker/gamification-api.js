...
async function insertLedger(env,event){
  const metadata=event.metadata||{},examMode=metadata.examMode||'';
  return env.DB.prepare(`INSERT OR IGNORE INTO gamification_point_ledger(id,user_id,season_id,attempt_id,test_id,event_type,points,status,idempotency_key,metadata_json)
    SELECT ?,?,?,?,?,?,?,?,?,?
    WHERE ?<>'awarded' OR (
      COALESCE((SELECT sum(points) FROM gamification_point_ledger WHERE user_id=? AND status='awarded' AND date(created_at,'+3 hours')=date('now','+3 hours')),0)+?<=COALESCE((SELECT total_daily_cap FROM gamification_settings WHERE id='platform'),350)
      AND (?<>'practice' OR COALESCE((SELECT sum(points) FROM gamification_point_ledger WHERE user_id=? AND status='awarded' AND json_extract(metadata_json,'$.examMode')='practice' AND date(created_at,'+3 hours')=date('now','+3 hours')),0)+?<=COALESCE((SELECT practice_daily_cap FROM gamification_settings WHERE id='platform'),180))
    )`).bind(event.id,event.userId,event.seasonId,event.attemptId||null,event.testId||null,event.eventType,event.points,event.status,event.idempotencyKey,JSON.stringify(metadata),event.status,event.userId,event.points,examMode,event.userId,event.points).run();
}
...
async function progressMissions(env,userId,season,test,sourceEventId){
  ...
  if(Number(activeDays?.value||0)>=Number(config.weeklyDaysTarget||3)){
    const eventId=crypto.randomUUID();const inserted=await insertLedger(env,{id:eventId,userId,seasonId:season.id,eventType:'weekly_activity',points:25,status:'awarded',idempotencyKey:`mission:${missionId}`,metadata:{missionId,sourceEventId}});
  ...
  if(String(test.subjectId||'')===String(weak.subjectId)){
    ...
    const eventId=crypto.randomUUID();const inserted=await insertLedger(env,{id:eventId,userId,seasonId:season.id,eventType:'weak_topic',points:20,status:'awarded',idempotencyKey:`mission:${weakId}`,metadata:{missionId:weakId,sourceEventId}});
...
export async function awardAttemptPoints(env,attempt,result){
  ...
  const inserted=await insertLedger(env,{id,userId:detail.userId,seasonId:season.id,attemptId:attempt.id,testId:detail.testId,eventType:'attempt',points,status,idempotencyKey:`attempt:${attempt.id}`,metadata:{...breakdown,examMode:detail.examMode,difficultyLevel:detail.difficultyLevel,attemptNumber,calibration,durationSeconds:duration}});
...
export async function handleGamificationApi(request,env,url,user){
  ...
  const review=url.pathname.match(/^\/api\/admin\/gamification\/reviews\/([^/]+)$/);if(review&&request.method==='PATCH'){
    const row=await env.DB.prepare(`SELECT l.id,l.user_id AS studentId,l.points,l.created_at AS createdAt,l.metadata_json AS metadata,u.university_id AS universityId,u.college_id AS collegeId,u.department_id AS departmentId,u.phase_id AS phaseId,u.section_id AS sectionId FROM gamification_point_ledger l JOIN users u ON u.id=l.user_id WHERE l.id=? AND l.status='pending_review'`).bind(review[1]).first();if(!row)return fail('NOT_FOUND','طلب المراجعة غير موجود',404);if(user.role!=='owner'&&!(await hasPermission(env,user,'review_gamification',academicContext(row))))return fail('FORBIDDEN','الطلب خارج نطاق صلاحيتك',403);const parsed=await readJsonBody(request);if(parsed.error)return fail(parsed.error.code,parsed.error.message,parsed.error.status);const decision=parsed.value?.decision,note=String(parsed.value?.note||'').trim();if(!['approve','void'].includes(decision)||note.length>500)return fail('VALIDATION','قرار المراجعة غير صالح');let updated;if(decision==='approve'){const mode=JSON.parse(row.metadata||'{}').examMode||'';updated=await env.DB.prepare(`UPDATE gamification_point_ledger SET status='awarded',reviewed_by=?,reviewed_at=CURRENT_TIMESTAMP,review_note=? WHERE id=? AND status='pending_review' AND COALESCE((SELECT sum(points) FROM gamification_point_ledger WHERE user_id=? AND status='awarded' AND date(created_at,'+3 hours')=date(?,'+3 hours')),0)+?<=COALESCE((SELECT total_daily_cap FROM gamification_settings WHERE id='platform'),350) AND (?<>'practice' OR COALESCE((SELECT sum(points) FROM gamification_point_ledger WHERE user_id=? AND status='awarded' AND json_extract(metadata_json,'$.examMode')='practice' AND date(created_at,'+3 hours')=date(?,'+3 hours')),0)+?<=COALESCE((SELECT practice_daily_cap FROM gamification_settings WHERE id='platform'),180))`).bind(user.id,note||null,row.id,row.studentId,row.createdAt,row.points,mode,row.studentId,row.createdAt,row.points).run();if(!Number(updated.meta?.changes))return fail('DAILY_CAP','لا يمكن اعتماد النقاط لأنها تتجاوز الحد اليومي',409)}else updated=await env.DB.prepare(`UPDATE gamification_point_ledger SET status='void',reviewed_by=?,reviewed_at=CURRENT_TIMESTAMP,review_note=? WHERE id=? AND status='pending_review'`).bind(user.id,note||null,row.id).run();await env.DB.prepare(`INSERT INTO audit_logs(entity,entity_id,action,by_user_id,details_json) VALUES('gamification',?,?,?,?)`).bind(row.id,decision==='approve'?'points_approved':'points_voided',user.id,JSON.stringify({note})).run();return json({updated:Boolean(updated.meta?.changes)})
  }
