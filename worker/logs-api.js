...
import {escapeLike,secureHeaders} from './security.js';
...
export async function handleLogsApi(request,env,url,user){
  ...
  const limit=Math.min(500,Math.max(25,Number(url.searchParams.get('limit'))||200));const q=String(url.searchParams.get('q')||'').trim().slice(0,80);const like=`%${escapeLike(q)}%`;
  ...
  if(url.pathname==='/api/admin/logs/accounts'){
    ...
    await env.DB.prepare(`DELETE FROM account_events WHERE at<datetime('now','-12 months')`).run();const rows=await env.DB.prepare(`SELECT e.id,e.account_code AS accountCode,e.event_type AS eventType,e.outcome,e.device_hash AS deviceHash,e.at,e.details_json AS detailsJson,u.name,u.email,u.account_role AS role,v.name AS universityName,c.name AS collegeName,COALESCE(d.display_name,d.name) AS departmentName FROM account_events e LEFT JOIN users u ON u.id=e.user_id LEFT JOIN universities v ON v.id=u.university_id LEFT JOIN colleges c ON c.id=u.college_id LEFT JOIN departments d ON d.id=u.department_id WHERE (?='' OR u.name LIKE ? ESCAPE '\\' OR u.email LIKE ? ESCAPE '\\' OR e.account_code LIKE ? ESCAPE '\\') ORDER BY e.at DESC LIMIT ?`).bind(q,like,like,like,limit).all();return json({data:rows.results,retentionMonths:12});
  }
  return null;
}
