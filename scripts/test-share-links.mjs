...
const testRequest=new Request('https://example.test/test/demo-preop');const testResponse=await serveSharePage(testRequest,env,new URL(testRequest.url),shell());const testHtml=await testResponse.text();if(!testHtml.includes('تقييم المريض قبل العملية | MedExam')||!testHtml.includes('/test/demo-preop')||!testHtml.includes('og:title'))throw new Error('Direct test metadata failed');
const lectureRequest=new Request('https://example.test/lecture/lec-a4-general-3');const lectureResponse=await serveSharePage(lectureRequest,env,new URL(lectureRequest.url),shell());const lectureHtml=await lectureResponse.text();if(!lectureHtml.includes('المحاضرة الثالثة: تقييم ما قبل العملية | MedExam')||!lectureHtml.includes('/lecture/lec-a4-general-3'))throw new Error('Direct lecture metadata failed');
