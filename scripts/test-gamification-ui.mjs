...
assert(app.includes("['points',Trophy,'نبضي']")&&app.includes("adminTab==='gamification'"),'gamification destinations must be reachable');
for(const text of ['نقاط للإتقان والتحسن والمراجعة، لا للسرعة.','لوحة المنافسة الهادئة','إخفائي من المنافسة','هدف تعاوني اختياري للشعبة'])assert(student.includes(text),`missing student gamification UX: ${text}`);
