...
function denied(user){return user?.role==='admin'?null:fail('FORBIDDEN','هذه العملية للمشرف فقط',403)}
...
export async function handleCatalogAdminApi(request,env,url,user){
  ...
  if(url.pathname==='/api/admin/departments'&&request.method==='POST'){
    const deny=denied(user);if(deny)return deny;const value=await readBody(request);if(!validName(value?.name))return fail('VALIDATION','اسم القسم يجب أن يكون بين حرفين و120 حرفًا');
  ...
  if(department&&request.method==='PATCH'){
    const deny=denied(user);if(deny)return deny;const value=await readBody(request);if(!validName(value?.name))return fail('VALIDATION','اسم القسم غير صالح');
  ...
  if(department&&request.method==='DELETE'){
    ...
    const result=await env.DB.prepare(`DELETE FROM departments WHERE id=?`).bind(department[1]).run();if(!result.meta.changes)return fail('NOT_FOUND','القسم غير موجود',404);await audit(env,user,'department',department[1],'delete',{});return new Response(null,{status:204});
  }

  if(url.pathname==='/api/admin/phases'&&request.method==='POST'){
    const deny=denied(user);if(deny)return deny;const value=await readBody(request);if(!validName(value?.name)||!value?.departmentId)return fail('VALIDATION','اسم المرحلة والقسم مطلوبان');
  ...
  if(phase&&request.method==='PATCH'){
    const deny=denied(user);if(deny)return deny;const value=await readBody(request);if(!validName(value?.name))return fail('VALIDATION','اسم المرحلة غير صالح');
  ...
  if(phase&&request.method==='DELETE'){
    ...
    const result=await env.DB.prepare(`DELETE FROM phases WHERE id=?`).bind(phase[1]).run();if(!result.meta.changes)return fail('NOT_FOUND','المرحلة غير موجودة',404);await audit(env,user,'phase',phase[1],'delete',{});return new Response(null,{status:204});
  }

  if(url.pathname==='/api/admin/subjects'&&request.method==='POST'){
    const deny=denied(user);if(deny)return deny;const value=await readBody(request);if(!validName(value?.name)||!value?.phaseId)return fail('VALIDATION','اسم المادة والمرحلة مطلوبان');
  ...
  if(subject&&request.method==='PATCH'){
    const deny=denied(user);if(deny)return deny;const value=await readBody(request);if(!validName(value?.name))return fail('VALIDATION','اسم المادة غير صالح');
  ...
  if(subject&&request.method==='DELETE'){
    ...
    const result=await env.DB.prepare(`DELETE FROM subjects WHERE id=?`).bind(subject[1]).run();if(!result.meta.changes)return fail('NOT_FOUND','المادة غير موجودة',404);await audit(env,user,'subject',subject[1],'delete',{});return new Response(null,{status:204});
  }

  if(url.pathname==='/api/admin/lectures'&&request.method==='POST'){
    const deny=denied(user);if(deny)return deny;const value=await readBody(request);if(!validName(value?.name)||!value?.subjectId)return fail('VALIDATION','اسم المحاضرة والمادة مطلوبان');
  ...
  if(lecture&&request.method==='PATCH'){
    const deny=denied(user);if(deny)return deny;const value=await readBody(request);if(!validName(value?.name))return fail('VALIDATION','اسم المحاضرة غير صالح');
  ...
  if(lecture&&request.method==='DELETE'){
    ...
    const result=await env.DB.prepare(`DELETE FROM lectures WHERE id=?`).bind(lecture[1]).run();if(!result.meta.changes)return fail('NOT_FOUND','المحاضرة غير موجودة',404);await audit(env,user,'lecture',lecture[1],'delete',{});return new Response(null,{status:204});
  }
  return null;
}
