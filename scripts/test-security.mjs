...
const csp=anonymous.headers.get('content-security-policy')||'';if(!csp.includes("style-src 'self'")||csp.includes("style-src 'self' 'unsafe-inline'")||!csp.includes("style-src-attr 'unsafe-inline'")||anonymous.headers.get('x-frame-options')!=='DENY'||anonymous.headers.get('strict-transport-security')===null||anonymous.headers.get('cross-origin-resource-policy')!=='same-origin'||anonymous.headers.get('origin-agent-cluster')!=='?1'||anonymous.headers.get('x-permitted-cross-domain-policies')!=='none')throw new Error('Security headers are missing or CSP is too permissive');
