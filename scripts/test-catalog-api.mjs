...
const softDelete=await call(`/api/admin/departments/${department.data.id}`,'DELETE');if(softDelete.status!==200||!softDelete.data.batchId)throw new Error('Owner soft delete failed');
const hiddenItems=sqlite.prepare(`SELECT resource_type AS type,resource_id AS id FROM academic_deleted_items WHERE batch_id=?`).all(softDelete.data.batchId);if(!hiddenItems.some(item=>item.type==='department'&&item.id===department.data.id)||!hiddenItems.some(item=>item.type==='phase'&&item.id===phase.data.id))throw new Error('Soft delete did not include descendants');
const restored=await call(`/api/admin/academic-trash/${softDelete.data.batchId}`,'POST',{});if(restored.status!==200||sqlite.prepare(`SELECT count(*) AS count FROM academic_deleted_items WHERE batch_id=?`).get(softDelete.data.batchId).count!==0)throw new Error('Academic restore failed');
...
console.log(JSON.stringify({ok:true,department:department.data.id,phase:phase.data.id,subject:subject.data.id,lectureCount:Number(count),softDelete:softDelete.status,restored:restored.status,copiedCollege:copied.data.id,copiedCounts,copiedTests,duplicateBlocked:duplicate.status}));
