...
const newLogin=await complete(await start(),await googleToken(googleEmail,googlePassword));assert.equal(newLogin.response.status,202);assert.equal(newLogin.data.status,'pending');const newUser=sqlite.prepare(`SELECT account_role,account_status FROM users WHERE email=?`).get(googleEmail);assert.equal(newUser.account_role,'student');assert.equal(newUser.account_status,'pending');
assert.equal((await complete(await start(),await googleToken(googleEmail,googlePassword))).response.status,403);
...
console.log(JSON.stringify({ok:true,passwordPolicy:true,baghdadTime:true,resetRevokesSessions:true,oneUseReset:true,emailActions:true,google:{firebaseHandler:true,session:true,replayRejected:true,expiredRejected:true,pending:true,suspended:true,noEmailTakeover:true,noUnverifiedLogin:true,providerVerified:true,csrf:true}}));
