...
export async function serveSharePage(request,env,url,indexResponse){
  ...
  const canonical=`${PUBLIC_ORIGIN}/${match[1]}/${encodeURIComponent(id)}`;const imageUrl=`${PUBLIC_ORIGIN}/og.png`;const safeTitle=escapeHtml(title);const safeDescription=escapeHtml(description);const safeCanonical=escapeHtml(canonical);const safeImage=escapeHtml(imageUrl);
