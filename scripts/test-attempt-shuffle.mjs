...
function headers(json=false){const value={'oai-authenticated-user-id':userId,'oai-authenticated-user-email':'shuffle@example.com','origin':'https://example.test','sec-fetch-site':'same-origin'};if(json)value['content-type']='application/json';return value}
...
await call('/api/me');await call('/api/me/profile','PATCH',{universityId:'uni-eratrans',collegeId:'college-eratrans-medical',departmentId:'dep-anesthesia',phaseId:'pha-a4'});const first=await begin();await answerCorrectly(first);const second=await begin();
