const PUBLIC_ORIGIN='https://medexam-iraq-2026.hussein2004810.chatgpt.site';
...
export async function serveSharePage(request,env,url,indexResponse){
  ...
  let title='MedExam — منصة الاختبارات الطبية';let description='اختبارات طبية تفاعلية للطلاب مع حفظ النتائج ومراجعة الإجابات.';
  if(match[1]==='test'){
    ...
    if(test){title=`${test.title} | MedExam`;description=`اختبار ${test.subjectName} — ${test.lectureName}. ${test.questionCount} أسئلة خلال ${test.durationMinutes} دقيقة.`}
  }else{
    ...
    if(lecture){title=`${lecture.lectureName} | MedExam`;description=`${lecture.subjectName} — ${lecture.departmentName}، ${lecture.phaseName}. ${lecture.testCount} اختبارات متاحة.`}
  }
  const canonical=`${PUBLIC_ORIGIN}/${match[1]}/${encodeURIComponent(id)}`;const safeTitle=escapeHtml(title);const safeDescription=escapeHtml(description);const safeCanonical=escapeHtml(canonical);
