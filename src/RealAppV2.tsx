...
export default function RealAppV2(){
  ...
  {view==='home'&&<><div className="bentoGrid">
    <article className="bentoCard bentoSpan8 bentoHero">
      <div><small className="bentoBadge">مرحبًا بعودتك</small><h2>{user.name}</h2><p>مادتك، محاضرتك، ثم اختبارك — KIUR التابعة لقناة ERATRANS</p><button className="solid" onClick={()=>navigate('tests')}><ClipboardList/>ابدأ اختبارك الآن</button></div>
      <div className="bentoHeroVitals"><MedicalVitals compact/><span className="dateChip">{new Intl.DateTimeFormat('ar-IQ',{dateStyle:'long'}).format(new Date())}</span></div>
    </article>
    <article className="bentoCard bentoSpan4 bentoQuickActions">
      <h3>وصول سريع</h3>
      <button type="button" onClick={()=>{setSmartReviewTab('flashcards');setSmartReviewOpen(true)}}><Brain/>المراجعة الذكية والتكرار</button>
      <button type="button" onClick={()=>setCommandPaletteOpen(true)}><Search/>البحث والتنقل <kbd>⌘K</kbd></button>
      <button type="button" onClick={()=>navigate('history')}><History/>سجل النتائج</button>
      <button type="button" onClick={()=>navigate('glimpses')}><HeartPulse/>اللمحات السريرية</button>
      {user.role==='student'&&<button type="button" onClick={()=>navigate('points')}><Trophy/>نقاطي وإنجازاتي</button>}
    </article>
  </div>
  {!studyLoading&&!studyError&&<div className="bentoStatsGrid"><article className="bentoStatCard"><span className="statIcon"><CheckCircle2/></span><div><b>{completed}</b><small>محاولات مكتملة</small></div></article><article className="bentoStatCard"><span className="statIcon amber"><BarChart3/></span><div><b>{summary.averagePercentage}%</b><small>متوسط النتائج</small></div></article><article className="bentoStatCard"><span className="statIcon blue"><ShieldCheck/></span><div><b>{passed}</b><small>اختبارات ناجحة</small></div></article><article className="bentoStatCard"><span className="statIcon coral"><BookOpen/></span><div><b>{tests.length}</b><small>اختبارات متاحة</small></div></article></div>}
  {studyContent}
  ...
  {view==='profile'&&<><Title title="حسابي" subtitle={user.role==='student'?'هويتك الدراسية، تقدمك، شهاداتك وأمان حسابك في مكان واحد.':'بيانات حسابك ونطاق عملك داخل KIUR.'}/>{user.role==='student'?<Suspense fallback={<SectionLoading/>}><StudentProfile learningHub={learningHub} notify={notify} onCorrectPath={()=>setProfileSetup(true)} onLogout={()=>void logoutCurrent()}/></Suspense>:<AccountProfile user={user} onLogout={()=>void logoutCurrent()}/>}</>}
{view==='admin'&&staff&&<>
