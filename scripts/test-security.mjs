...
const forged=await worker.fetch(new Request('https://kiur.example/api/me',{headers:{'oai-authenticated-user-id':'forged-owner','oai-authenticated-user-email':'forged@example.com'}}),env);if(forged.status!==401)throw new Error('Production-like host trusted spoofed ChatGPT identity headers');
const missingOrigin=await worker.fetch(new Request('https://example.test/api/me/profile',{method:'PATCH',headers:{'content-type':'application/json','oai-authenticated-user-id':admin.id,'oai-authenticated-user-email':admin.email},body:'{}'}),env);if(missingOrigin.status!==403)throw new Error('Mutation without an Origin header was accepted');
const crossSite=await worker.fetch(new Request('https://example.test/api/me/profile',{method:'PATCH',headers:{'content-type':'application/json','origin':'https://attacker.test','sec-fetch-site':'cross-site','oai-authenticated-user-id':admin.id,'oai-authenticated-user-email':admin.email},body:'{}'}),env);if(crossSite.status!==403)throw new Error('Cross-site mutation was accepted');

const owner={id:'student-owner',email:'owner@example.com'};const other={id:'student-other',email:'other@example.com'};
for(const profileUser of [owner,other]){await call('/api/me',{user:profileUser});const profiled=await call('/api/me/profile',{method:'PATCH',user:profileUser,body:{universityId:'uni-eratrans',collegeId:'college-eratrans-medical',departmentId:'dep-anesthesia',phaseId:'pha-a4'}});if(profiled.status!==200)throw new Error('Student profile setup failed')}
const lockedProfile=await call('/api/me/profile',{method:'PATCH',user:owner,body:{universityId:'uni-eratrans',collegeId:'college-eratrans-medical',departmentId:'dep-anesthesia',phaseId:'pha-a4'}});if(lockedProfile.status!==409)throw new Error('Student could change an already saved academic profile');
...
const csp=anonymous.headers.get('content-security-policy')||'';if(!csp.includes("style-src 'self'")||csp.includes("style-src 'self' 'unsafe-inline'")||!csp.includes("style-src-attr 'unsafe-inline'")||!csp.includes('https://apis.google.com')||!csp.includes('https://www.gstatic.com')||!csp.includes('https://www.googleapis.com')||anonymous.headers.get('x-frame-options')!=='DENY'||anonymous.headers.get('strict-transport-security')===null||anonymous.headers.get('cross-origin-resource-policy')!=='same-origin'||anonymous.headers.get('origin-agent-cluster')!=='?1'||anonymous.headers.get('x-permitted-cross-domain-policies')!=='none')throw new Error('Security headers are missing or CSP is too permissive');
